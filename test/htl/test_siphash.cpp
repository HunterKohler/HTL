...
TEST(SipHashTest, Hash)
{
    std::vector<std::uint8_t> key(SipHash::key_size);
    std::vector<std::uint8_t> out(SipHash::digest_size);
    ...
    for (int i = 0; i < 64; i++) {
        auto &digest = siphash_test_digests[i];

        SipHash::hash(message.data(), i, key.data(), out.data());
